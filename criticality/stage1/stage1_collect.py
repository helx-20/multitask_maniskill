...
def main(args):
    ...
    if args.task_id is not None:
        ...
        raise ValueError("Must pass --task_id or --env_id")
    ...
    for ep in range(args.n):
        ...
        if (ep + 1) % 10 == 0:
            print(f"[{ep+1}/{args.n}] pos={len(pos)} neg={len(neg)} "
...
def _save(args, pos, neg):
    """Per-task filename includes task short_name to avoid collisions when
    multiple workers across multiple tasks write to the same directory."""
